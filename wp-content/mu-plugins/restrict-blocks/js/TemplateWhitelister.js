...
const { isEditorPanelOpened, getActiveGeneralSidebarName } = select(
  'core/edit-post'
)
...
templateWhitelistNotice.innerText = __(
  'Some page templates are currently unavailable because they are incompatible with the following blocks on this page. You will need to remove them in order to make those templates available again.',
  'ufhealth-restrict-blocks'
)
...
class TemplateWhitelister {
  ...
  constructor(restrictedBlocks) {
    this.defaultPageTemplates = select(
      'core/editor'
  ...
  run() {
    ...
    const { templates, restrictedBlocks } = this.checkForRestrictedBlocks(
      blocks
    )
  ...
  updateWhitelistedTemplates(templates) {
    if (templates.length > 0) {
      ...
      updateEditorSettings({
        availableTemplates: pick(this.defaultPageTemplates, templates),
      })
    } else {
  ...
  addOrRemoveWhitelistNotice(restrictedBlocks) {
    if (
      isEditorPanelOpened('page-attributes') &&
      getActiveGeneralSidebarName() === 'edit-post/document'
    ) {
